...
BUILD_COMMAND=$1
if [ "$BUILD_COMMAND" == "c7lte_chn" ]; then
	BUILD_PRODUCT=c7lte_chn_open
	PRODUCT_NAME=c7ltechn
	SIGN_MODEL=SM-C7000_CHN_CHC_ROOT0
elif [ "$BUILD_COMMAND" == "c7lte_kor_skt" ]; then
	BUILD_PRODUCT=c7lte_kor_skt
	PRODUCT_NAME=c7lteskt
	SIGN_MODEL=
fi
...
FUNC_BUILD_KERNEL()
{
	...
	make -C $BUILD_KERNEL_DIR O=$BUILD_KERNEL_OUT_DIR -j$BUILD_JOB_NUMBER ARCH=arm64 \
			...
			SELINUX_DEFCONFIG=$SELINUX_DEFCONFIG \
			SELINUX_LOG_DEFCONFIG=$SELINUX_LOG_DEFCONFIG \
			DMVERITY_DEFCONFIG=$DMVERITY_DEFCONFIG || exit -1
...
SECFUNC_PRINT_HELP()
{
	...
	echo "$0 \$1 \$2 \$3"
	echo "  \$1 : "
	echo "	for C7LTE_CHN_OPEN use c7lte_chn"
	echo "	for C7LTE_KOR_SKT use c7lte_kor_skt"
	echo "  \$2 : "
