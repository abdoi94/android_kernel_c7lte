...
BUILD_COMMAND=$1
if [ "$BUILD_COMMAND" == "msm895332lte_chn" ]; then
	BUILD_PRODUCT=msm895332lte_chn_open
	PRODUCT_NAME=msm895332ltechn;
	SIGN_MODEL=
fi
...
KERNEL_DEFCONFIG=msm895332_sec_defconfig
DEBUG_DEFCONFIG=msm895332_sec_eng_defconfig
#DEBUG_DEFCONFIG=msm895332_sec_eng_defconfig
...
BUILD_CONF=${BUILD_CONF_PATH}/build_conf.${BUILD_PRODUCT}
...
KERNEL_ZIMG=$BUILD_KERNEL_OUT_DIR/arch/arm/boot/Image
...
FUNC_BUILD_DTIMAGE_TARGET()
{
	...
	cp $BUILD_KERNEL_OUT_DIR/arch/arm/boot/dts/samsung/*.dtb $BUILD_KERNEL_OUT_DIR/arch/arm/boot/dts/.	
...
FUNC_BUILD_KERNEL()
{
	...
	make -C $BUILD_KERNEL_DIR O=$BUILD_KERNEL_OUT_DIR -j$BUILD_JOB_NUMBER ARCH=arm \
			CROSS_COMPILE=$BUILD_CROSS_COMPILE $KERNEL_DEFCONFIG || exit -1
...
SECFUNC_PRINT_HELP()
{
	...
	echo "$0 \$1 \$2 \$3"
	echo "  \$1 : "
	echo "	for MSM8953LTE_CHN use msm8953lte_chn"
	echo "  \$2 : "
