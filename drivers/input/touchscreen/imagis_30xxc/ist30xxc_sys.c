...
int ts_power_enable(struct ist30xx_data *data, int en)
{
	...
	static struct regulator *avdd = NULL, *vddo = NULL;
	int retval = 0;

	if (!avdd) {
		avdd = devm_regulator_get(dev, "avdd");
		if (IS_ERR(avdd)) {
			tsp_err("%s: could not get avdd, rc = %ld\n",
		...
		if (retval)
			tsp_err("%s: unable to set avdd voltage to 3.3V\n",
				__func__);
	}
	if (!vddo) {
		...
		if (retval)
			tsp_err("%s: unable to set vddo voltage to 1.8V\n",
				__func__);
	}
	...
	if (en) {
		...
		if (retval) {
			tsp_err("%s: Failed to enable vddo: %d\n", __func__, retval);
			return retval;
		}
	} else {
		retval = regulator_disable(vddo);
		if (retval) {
			tsp_err("%s: Failed to disable vddo: %d\n", __func__, retval);
			return retval;
		}
		retval = regulator_disable(avdd);
		if (retval) {
			tsp_err("%s: Failed to disable avdd: %d\n", __func__, retval);
			return retval;
		}
	}

	tsp_info("%s %s: avdd %s, vddo %s\n", __func__, (en) ? "on" : "off",
			regulator_is_enabled(avdd) ? "on" : "off",
